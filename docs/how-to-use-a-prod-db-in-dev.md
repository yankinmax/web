...
2. Create and download the dump
    ...
    ```
    pg_dump --format=c -h localhost -p 5555 -U old_dream_6085 old_dream_6085 -O --file /path/to/backups/mtsmte-$(date +%Y-%m-%d).pg
    ```
...
2. Load the production dump :
    ...
    ```
    docker-compose run --rm odoo pg_restore -p 5432 -d prod < /path/to/backups/mtsmte-$(date +%Y-%m-%d).pg
    ```
