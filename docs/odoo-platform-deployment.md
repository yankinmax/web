...
* `docker-compose.yml`
 ...
 ```
 odoo:
  image: camptocamp/mtsmte_odoo:10.0.0
...
* `rancher.public.env`
 ...
 ```
 export DOMAIN_NAME=integration.mtsmte.odoo.camptocamp.ch
 ...
 export AWS_BUCKETNAME=mtsmte-odoo-integration
 
 export ODOO_SESSION_REDIS=1
 export ODOO_SESSION_REDIS_HOST=redis
 export ODOO_SESSION_REDIS_PREFIX=mtsmte-odoo-integration
...
* `docker-compose.yml`
 ...
 letsencrypt:
   ...
   image: janeczku/rancher-letsencrypt:v0.3.0
   volumes:
     - mtsmte-prod-certs:/etc/letsencrypt/production/certs
 ```
...
* `rancher.public.env`
 ...
 export AWS_BUCKETNAME=mtsmte-odoo-prod
 
 export ODOO_SESSION_REDIS=1
 export ODOO_SESSION_REDIS_HOST=redis
 export ODOO_SESSION_REDIS_PREFIX=mtsmte-odoo-prod
...
```
./rancher mtsmte-odoo-integration up -d letsencrypt
./rancher mtsmte-odoo-prod up -d letsencrypt
```
...
* Use the local `./rancher` script, example to start the containers:
  ...
  ```
  ./rancher mtsmte-odoo-integration up -d
  ```
...
* Use the local `./rancher` script, example to upgrade the containers:
  ...
  ```
  ./rancher mtsmte-odoo-integration up --pull --recreate --confirm-upgrade -d
  ```
