...
@anthem.log
def sale_config_settings_MT(ctx):
    """ Setup sale.config.settings for Metallo-Tests S.A. """
    model = ctx.env['sale.config.settings'].with_context({
        'tracking_disable': 1
    })  # noqa
    with open(  # Handmade. To be replaced with proper export into a file
            "./odoo/data/install/generated/sale_config_sale_note_mt.txt"
    ) as sale_note_mt:
        model.create({
            # Default Terms and Conditions *  # noqa
            # Handmade. To be replaced with proper export into a file
            'sale_note': sale_note_mt.read(),
            ...
            # Online Quotations: Send quotations your customer can approve & pay online (advanced)  # noqa
            'module_website_quote': 1,
            # Order Routing: Choose specific routes on sales order lines (advanced)  # noqa
            'group_route_so_lines': 1,
            # Company  # noqa
            'company_id': ctx.env.ref('__setup__.company_mte').id,  # noqa
...
@anthem.log
def sale_config_settings_MTS(ctx):
    """ Setup sale.config.settings for MTS Materiaux Technologies Surfaces SA
    """
    model = ctx.env['sale.config.settings'].with_context({
        'tracking_disable': 1
    })  # noqa
    with open(  # Handmade. To be replaced with proper export into a file
            "./odoo/data/install/generated/sale_config_sale_note_mts.txt"
    ) as sale_note_mts:
        model.create({
            # Default Terms and Conditions *  # noqa
            'sale_note': sale_note_mts.read(),
            # Tax Display: Show line subtotals without taxes (B2B)  # noqa
            'sale_show_tax': 'subtotal',
            # Sales Safety Days *  # noqa
            'security_lead': 0.0,
...
def sale_config_settings(ctx):
    sale_config_settings_MT(ctx)
    sale_config_settings_MTS(ctx)
